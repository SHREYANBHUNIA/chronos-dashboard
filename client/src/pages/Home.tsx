...
function TracesView({ activeTraceId, setActiveTraceId, setView, setCompareTraceId }: { activeTraceId: string; setActiveTraceId: (id: string) => void; setView: (view: WorkspaceView) => void; setCompareTraceId: (id: string) => void }) {
  ...
  const [severityFilter, setSeverityFilter] = useState<"all" | TraceRecord["severity"]>("all");
  const [serviceFilter, setServiceFilter] = useState("all");
  const [environmentFilter, setEnvironmentFilter] = useState("all");
  const [statusFilter, setStatusFilter] = useState<"all" | "errors" | "success">("all");
  const activeFilterCount = [severityFilter !== "all", serviceFilter !== "all", environmentFilter !== "all", statusFilter !== "all"].filter(Boolean).length;
  const filtered = traceRecords.filter((trace) => {
    const matchesQuery = `${trace.title} ${trace.service} ${trace.route} ${trace.environment}`.toLowerCase().includes(query.toLowerCase());
    const matchesSeverity = severityFilter === "all" || trace.severity === severityFilter;
    const matchesService = serviceFilter === "all" || trace.service === serviceFilter;
    const matchesEnvironment = environmentFilter === "all" || trace.environment === environmentFilter;
    const matchesStatus = statusFilter === "all" || (statusFilter === "errors" ? trace.status >= 400 : trace.status < 400);
    return matchesQuery && matchesSeverity && matchesService && matchesEnvironment && matchesStatus;
  });
  return <div className="mx-auto max-w-[1320px] px-4 py-5 sm:px-6 lg:px-8 lg:py-7"><section className="mb-7 flex flex-col justify-between gap-5 xl:flex-row xl:items-end"><div><div className="flex items-center gap-2"><span className="technical-label text-[#35d6e8]">TRACE ARCHIVE / 04</span><span className="h-1 w-1 rounded-full bg-slate-600" /><span className="technical-label text-slate-500">captured requests</span></div><h1 className="mt-2 text-[28px] font-semibold tracking-[-0.04em] text-slate-100 sm:text-[34px]">Recorded traces</h1><p className="mt-2 max-w-2xl text-[13px] leading-relaxed text-slate-400">Select a request to reconstruct its execution, or stage any two traces for a variable-level divergence study.</p></div><div className="flex items-center gap-2"><div className="flex items-center gap-2 border border-white/[0.1] bg-[#0c1014]/80 px-3 py-2"><Search className="h-3.5 w-3.5 text-slate-500" /><input aria-label="Filter traces" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Filter traces" className="w-32 bg-transparent text-[11px] text-slate-200 outline-none placeholder:text-slate-600 sm:w-44" /></div><div className="relative"><button aria-label="Open trace filters" aria-expanded={filterOpen} className={`relative grid h-9 w-9 place-items-center border bg-white/[0.03] text-slate-400 transition hover:bg-white/[0.07] hover:text-white ${filterOpen || activeFilterCount ? "border-[#35d6e8]/50 text-[#35d6e8]" : "border-white/[0.1]"}`} onClick={() => setFilterOpen((open) => !open)}><ListFilter className="h-4 w-4" />{activeFilterCount > 0 && <span className="absolute -right-1 -top-1 grid h-4 min-w-4 place-items-center rounded-full bg-[#35d6e8] px-1 mono text-[8px] font-semibold text-[#061619]">{activeFilterCount}</span>}</button>{filterOpen && <div className="absolute right-0 top-11 z-30 w-[280px] border border-white/[0.14] bg-[#11171c] p-4 shadow-2xl shadow-black/40"><div className="flex items-center justify-between border-b border-white/[0.08] pb-3"><div><p className="technical-label text-slate-300">Filter traces</p><p className="mt-1 text-[10px] text-slate-600">Narrow the evidence archive</p></div><button className="text-slate-500 transition hover:text-slate-100" onClick={() => setFilterOpen(false)} aria-label="Close filters"><X className="h-3.5 w-3.5" /></button></div><div className="space-y-4 pt-4"><div><p className="technical-label mb-2 text-[8px] text-slate-600">severity</p><div className="grid grid-cols-2 gap-1">{(["all", "critical", "watch", "nominal"] as const).map((option) => <button key={option} className={`flex items-center gap-2 px-2 py-2 text-left text-[10px] transition ${severityFilter === option ? "bg-[#35d6e8]/[0.1] text-[#8deaf2]" : "text-slate-400 hover:bg-white/[0.05] hover:text-slate-200"}`} onClick={() => setSeverityFilter(option)}>{severityFilter === option ? <Check className="h-3 w-3" /> : <span className="h-3 w-3" />}{option}</button>)}</div></div><div><p className="technical-label mb-2 text-[8px] text-slate-600">service</p><select value={serviceFilter} onChange={(event) => setServiceFilter(event.target.value)} className="w-full border border-white/[0.1] bg-[#0c1014] px-2.5 py-2 text-[10px] text-slate-300 outline-none focus:border-[#35d6e8]/50"><option value="all">all services</option>{Array.from(new Set(traceRecords.map((trace) => trace.service))).map((service) => <option key={service} value={service}>{service}</option>)}</select></div><div><p className="technical-label mb-2 text-[8px] text-slate-600">environment</p><select value={environmentFilter} onChange={(event) => setEnvironmentFilter(event.target.value)} className="w-full border border-white/[0.1] bg-[#0c1014] px-2.5 py-2 text-[10px] text-slate-300 outline-none focus:border-[#35d6e8]/50"><option value="all">all environments</option>{Array.from(new Set(traceRecords.map((trace) => trace.environment))).map((environment) => <option key={environment} value={environment}>{environment}</option>)}</select></div><div><p className="technical-label mb-2 text-[8px] text-slate-600">response</p><div className="grid grid-cols-3 gap-1">{(["all", "errors", "success"] as const).map((option) => <button key={option} className={`px-2 py-2 text-[10px] transition ${statusFilter === option ? "bg-[#35d6e8]/[0.1] text-[#8deaf2]" : "text-slate-400 hover:bg-white/[0.05] hover:text-slate-200"}`} onClick={() => setStatusFilter(option)}>{option}</button>)}</div></div></div><button className="mt-4 w-full border-t border-white/[0.08] pt-3 text-left text-[10px] text-slate-500 transition hover:text-slate-200" onClick={() => { setSeverityFilter("all"); setServiceFilter("all"); setEnvironmentFilter("all"); setStatusFilter("all"); setQuery(""); }}>Clear all filters</button></div>}</div></div></section><div className="mb-6 grid gap-3 sm:grid-cols-3"><div className="hairline-card dot-grid p-4"><p className="technical-label text-[9px] text-slate-600">active archive</p><p className="mt-2 text-[22px] font-semibold text-slate-100">{traceRecords.length}</p><p className="mt-1 text-[10px] text-slate-500">requests retained in fixture store</p></div><div className="hairline-card dot-grid p-4"><p className="technical-label text-[9px] text-slate-600">semantic events</p><p className="mt-2 text-[22px] font-semibold text-[#35d6e8]">{traceRecords.reduce((sum, trace) => sum + trace.events.length, 0)}</p><p className="mt-1 text-[10px] text-slate-500">state-changing points indexed</p></div><div className="hairline-card dot-grid p-4"><p className="technical-label text-[9px] text-slate-600">diff engine</p><p className="mt-2 text-[22px] font-semibold text-[#f1b55a]">ready</p><p className="mt-1 text-[10px] text-slate-500">pair any two traces below</p></div></div><div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2"><Waypoints className="h-4 w-4 text-[#35d6e8]" /><p className="technical-label text-slate-400">Captured request ledger</p></div><span className="mono text-[10px] text-slate-600">{filtered.length} / {traceRecords.length} visible</span></div><div className="space-y-3">{filtered.map((trace) => <TraceCard key={trace.id} trace={trace} active={trace.id === activeTraceId} onInspect={() => { setActiveTraceId(trace.id); setView("investigate"); }} onCompare={() => { setCompareTraceId(trace.id); setView("comparisons"); }} />)}</div>{filtered.length === 0 && <div className="chronos-panel p-10 text-center"><p className="technical-label text-slate-500">No matching traces</p><p className="mt-2 text-[12px] text-slate-400">Try a service, route, or environment name.</p></div>}</div>;
}
